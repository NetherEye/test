		...
		<div class="login">
			...
			<div class="box_wrap">
				<div class="box">
					...
					<div class="box_nav">
						<div class="box_nav_item" :class="loginType == '省' ? 'activeLogin' : ''" @click="changeLoginType('省')">
							<img src="../../../static/img/provincial/shengji.png"/>
							<div class="nav_title">省级入口</div>
						</div>
						<div class="box_nav_item" :class="loginType == '市州' ? 'activeLogin' : ''" @click="changeLoginType('市州')">
							<img src="../../../static/img/provincial/shiji.png"/>
							<div class="nav_title">市级入口</div>
						</div>
						<div class="box_nav_item" :class="loginType == '区县' ? 'activeLogin' : ''" @click="changeLoginType('区县')">
							<img src="../../../static/img/provincial/xianji.png"/>
							<div class="nav_title">县级入口</div>
						</div>
						<div class="box_nav_item" :class="loginType == '乡镇' ? 'activeLogin' : ''" @click="changeLoginType('乡镇')">
							<img src="../../../static/img/provincial/xiangji.png"/>
							<div class="nav_title">乡级入口</div>
						</div>
						<div class="box_nav_item" :class="loginType == '景区' ? 'activeLogin' : ''" @click="changeLoginType('景区')">
							<img src="../../../static/img/provincial/jingqu.png"/>
							<div class="nav_title">景区入口</div>
						</div>
					</div>
					<div class="box_content">
						<div class="form">
							...
							<div class="form_item">
								...
								<div class="form_item_right">
									<el-input v-model.trim="username" placeholder="用户名" size="small"></el-input>
								</div>
							</div>
							<div class="form_item">
								<div class="form_item_left"></div>
								<div class="form_item_right">
									<el-input type="password" v-model.trim="password" placeholder="请输入密码" size="small" class="input1">
	...
	methods: {
		...
		login(){
			...
			fetch.post(allApi.login, par, res => {
				if(res.code === 200){
					if(res.data.role != this.loginType){ // 登录账号和前端登录选中级别不一致
						this.$message({
							type: 'warning',
							message: '入口和账号不一致'
						})
					...
					}else if(this.loginType == '市州'){
						sessionStorage.setItem('activeNav', '/city');
						this.$router.push({name:'市级首页'});
						
					}else if(this.loginType == '区县'){
						sessionStorage.setItem('activeNav', '/county');
						this.$router.push({name:'县级首页'});
						
					}else if(this.loginType == '乡镇'){
						sessionStorage.setItem('activeNav', '/village');
						this.$router.push({name:'乡级公告'});
						
					}else if(this.loginType == '景区'){
						sessionStorage.setItem('activeNav', '/scenic');
						this.$router.push({name:'景区首页'});
...
.form_item:nth-child(3) .form_item_left{
